...
void SweptDiscretization2D::updateRemoteConstants2(unsigned char *buffer)
{
	...
	MPI_Win_fence((MPI_MODE_NOSTORE | MPI_MODE_NOSUCCEED), this->constantsWindow);
	*/
	MPI_Scatter((unsigned char*)sendingBuffer,remoteConstantsCount * n * n * sizeof(double),MPI_BYTE,(unsigned char*)remoteConstants,remoteConstantsCount * n * n * sizeof(double),MPI_BYTE,0,MPI_COMM_WORLD);
	if(pg.rank == 0)
	{
		MPI_Free_mem(sendingBuffer);					
	}
...
void SweptDiscretization2D::allGatherOutputToSocket(int dataPoint)
{
	...
	if(pg.rank == 0)
	{
		...
		for(int r=0;r<pg.mpiSize;r++)
		{
			...
			for(int j=1;j<n+1;j++)
			{
				for(int i=1;i<n+1;i++)
				{
					...
					float val  = (float)processing[index + constants + dataPoint];
        ...
        sendString.clear();
        //base64encode((const void*)resultArray,resultArraySize * sizeof(double),sendString);
		base64encode((const void*)resultArray,resultArraySize * sizeof(float),sendString);
        char messageLength[9];
        string message;
        sprintf(messageLength,"%8u",(int)sendString.size());
        message.append(messageLength);
        message.append(sendString);
...
void SweptDiscretization2D::allGatherAllOutputToSocket()
{
	...
	if(pg.rank == 0)
	{
		...
		int resultArraySize = w * h;
		if(socketResultArray == NULL)
			socketResultArray = (float*) malloc(resultArraySize * sizeof(float) * outputLength);
		...
		for(int r=0;r<pg.mpiSize;r++)
		{
			...
			for(int j=1;j<n+1;j++)
			{
				for(int i=1;i<n+1;i++)
				{
					...
					{
						float val  = (float)processing[index + constants + point];
						int resultIndex = (iGlobal + jGlobal * n * pg.xNodes) * outputLength + point;
						socketResultArray[resultIndex] = val;
					}
				}
			}		
		}
		sendString.clear();
		base64encode((const void*)socketResultArray,resultArraySize * sizeof(float) * outputLength,sendString);
		char messageLength[9];
        string message;
        sprintf(messageLength,"%8u",(int)sendString.size());
        message.append(messageLength);
...
void SweptDiscretization2D::allGatherAllOutputToSocket2()
{
...
double SweptDiscretization2D::calculate(int cycles)
{
	...
	{	
		...
		if((checkForRemoteCycles != -1) && (c % checkForRemoteCycles == 0))
		{
			int code = 0;
			string command = getRemoteCommand(&code);
			if(command.length() == 0)
			{
				//printf("No Remote Commands!!\n");
				remoteCommandCode = 0;
				break;
			}
			else
			...
			MPI_Bcast((void*)&this->remoteCommandCode,1,MPI_INT,0,MPI_COMM_WORLD);
			switch(remoteCommandCode)
			{
				case 0:
				{
					break;
				...
				case 1:
				{
					//this->updateRemoteConstants(constantsArrayBytes);					
					this->updateRemoteConstants2(constantsArrayBytes);					
					break;
				}
				case 2:
				{
					this->streamSolution = true;										
					break;
				}
				case 3:
				{
					this->streamSolution = false;
					break;
				}
			}
			if(this->streamSolution)
			{
				//this->allGatherAllOutputToSocket();
				this->allGatherAllOutputToSocket2();
			}
		}
